...
from net import resnet26
from data.imagenet_subsets import IMAGENET_A_MASK, IMAGENET_R_MASK, IMAGENET_D109_MASK,IMAGENET_V_MASK
...
class ResNetDomainNet126(torch.nn.Module):
    ...
    """
    def __init__(self, arch="resnet50", checkpoint_path=None, num_classes=126, bottleneck_dim=1024):
        super().__init__()
    ...
    def load_from_checkpoint_target(self, checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location="cpu")
        state_dict = dict()
        model_state_dict = checkpoint["state_dict"] if "state_dict" in checkpoint.keys() else checkpoint["model"]
        for name, param in model_state_dict.items():
...
class ResNetDomainNet126_2(torch.nn.Module):
    """
    Architecture used for DomainNet-126
    """
    def __init__(self, arch="resnet50", checkpoint_path=None, num_classes=126, bottleneck_dim=512):
        super().__init__()
...
def get_model(args, num_classes):
    if args.dset == "domainnet126":
        base_model = ResNetDomainNet126(arch=args.net, checkpoint_path=args.CKPT_PATH, num_classes=num_classes)
    else:
        try:
            # load model from torchvision
            base_model,transform = get_torchvision_model(args.net, weight_version=args.WEIGHTS)
            return base_model,transform
        except ValueError:
            try:
                # load model from timm
                base_model = get_timm_model(args.net)
            except ValueError:
                try:
                    # load some custom models
                    if args.net == "resnet26_gn":
                        ...
                        raise ValueError(f"Model {args.net} is not supported!")
                    logger.info(f"Successfully restored model '{args.net}' from: {args.CKPT_PATH}")
                except ValueError:
                    # load model from robustbench
                    dataset_name = args.dset.split("_")[0]
                    base_model = load_model(args.net, args.CKPT_DIR, dataset_name, ThreatModel.corruptions)
    return base_model.cuda()

def get_model_2(args, num_classes):
    if args.dset == "domainnet126":
        base_model = ResNetDomainNet126_2(arch=args.net, checkpoint_path=args.CKPT_PATH, num_classes=num_classes)
    else:
        try:
            # load model from torchvision
            base_model,transform = get_torchvision_model(args.net, weight_version=args.WEIGHTS)
